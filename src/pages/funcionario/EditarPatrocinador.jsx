...
export default function EditarPatrocinador() {
  ...
  const handleSubmit = async (e) => {
    ...
    setExito(true)
    setTimeout(() => navigate(`/funcionario/patrocinadores/${id}`), 1500)
  }
    ...
    <div className="min-h-screen bg-[#f5f5f5]">
      <header className="bg-[#1e3a6e] text-white px-4 py-3 flex items-center justify-between shadow-md">
        <div className="flex items-center gap-3">
          <button onClick={() => navigate(`/funcionario/patrocinadores/${id}`)}>
            <ArrowLeft size={20} />
          </button>
          <span className="font-bold text-base">Editar patrocinador</span>
        </div>
        ...
        <div className="bg-white rounded-2xl p-4 shadow-sm border border-gray-100 flex flex-col gap-3">
          ...
          {form.tipo_uid && (
            <div>
              ...
              <input type="text" value={form.uid} onChange={set('uid')}
